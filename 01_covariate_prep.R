rastlist <- list.files(path = "/Covariates/covars_5k", pattern='.tif$',  full.names=TRUE)
covars <- stack(rastlist)
plot(covars)
res(covars)
tmax <- getData(name = "worldclim",var = "tmax", res = 2.5, path = "/Volumes/Soushie/results/covars_5k")
tmax_m <- mean(tmax)
res(tmax)
e <- extent(covars)
tmax_m <- crop(tmax_m, e)
writeRaster(tmax_m, "/Volumes/Soushie/results/covars_5k/tmax_m.tif",format = 'GTiff', overwrite = T)
tmin <- getData(name = "worldclim",var = "tmin", res = 2.5, path = "/Volumes/Soushie/results/covars_5k")
tmin_m <- mean(tmin)
tmin_m <- crop(tmin_m, e)
writeRaster(tmin_m, "/Volumes/Soushie/results/covars_5k/tmin_m.tif",format = 'GTiff', overwrite = T)
ppt <- getData(name = "worldclim",var = "prec", res = 2.5, path = "/Volumes/Soushie/results/covars_5k")
ppt_m <- mean(ppt)
ppt_m <- crop(ppt_m, e)
writeRaster(ppt_m, "/Volumes/Soushie/results/covars_5k/ppt_m.tif",format = 'GTiff', overwrite = T)
